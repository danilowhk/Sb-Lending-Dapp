...
export function NewBorrowModal({
  isOpen,
  onRequestClose,
  contract,
  provider,
  ...
  if (signer) {
    sbLendingContract = new ethers.Contract(
      SbLendingAddress,
      sbLendingAbi,
      signer
    );
  ...
  async function handleSubmit(event: FormEvent) {
    ...
    if (contract === "WETH") {
      if (type == "borrow") {
        const WETHBorrowTx = await sbLendingContract.borrowERC20(
          WETHAddress,
          amount
        );
        await WETHBorrowTx.wait();
      } else {
        const WETHPayBackTx = await sbLendingContract.payBackERC20(
          WETHAddress,
          amount
        );
    ...
    if (contract === "DAI") {
      if (type == "borrow") {
        const DaiBorrowTx = await sbLendingContract.borrowERC20(
          DAIAddress,
          amount
        );
        await DaiBorrowTx.wait();
      } else {
        const DaiPayBackTx = await sbLendingContract.payBackERC20(
          DAIAddress,
          amount
        );
    ...
    if (contract === "SBL") {
      if (type == "borrow") {
        const SBLBorrowTx = await sbLendingContract.borrowERC20(
          SBLAddress,
          amount
        );
        await SBLBorrowTx.wait();
      } else {
        const SBLPayBackTx = await sbLendingContract.payBackERC20(
          SBLAddress,
          amount
        );
