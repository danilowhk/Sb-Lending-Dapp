...
export function NewDepositModal({
  ...
  async function handleSubmit(event: FormEvent) {
    ...
    if (contract === "WETH") {
      if (type == "deposit") {
        const WETHDepositTx = await sbLendingContract.depositERC20(
          WETHAddress,
          amount
        );
        await WETHDepositTx.wait();
      } else {
        const WETHWithdrawTx = await sbLendingContract.withdrawERC20(
          WETHAddress,
          amount
        );
    ...
    if (contract === "DAI") {
      if (type == "deposit") {
        const DaiDepositTx = await sbLendingContract.depositERC20(
          DAIAddress,
          amount
        );
        await DaiDepositTx.wait();
      } else {
        const DaiWithdrawTx = await sbLendingContract.withdrawERC20(
          DAIAddress,
          amount
        );
    ...
    if (contract === "SBL") {
      if (type == "deposit") {
        const SBLDepositTx = await sbLendingContract.depositERC20(
          SBLAddress,
          amount
        );
        await SBLDepositTx.wait();
      } else {
        const SBLWithdrawTx = await sbLendingContract.withdrawERC20(
          SBLAddress,
          amount
        );
