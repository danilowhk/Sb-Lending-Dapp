...
describe("sbLending", function () {
  ...
  it("Check Deposit", async function () {
    ...
    const WETHdepositBalance = await sbLending.ERC20DepositList(
      wethToken.address,
      metaMaskUserAddress
    );
    const DAIdepositBalance = await sbLending.ERC20DepositList(
      daiToken.address,
      metaMaskUserAddress
    );
    const SBLdepositBalance = await sbLending.ERC20DepositList(
      sblToken.address,
      metaMaskUserAddress
    );
    ...
    assert(
      WETHdepositBalance > 0 && DAIdepositBalance > 0 && SBLdepositBalance > 0
    );
  });
  ...
  it("Check Deposit & Withdraw", async function () {
    ...
    const WETHdepositBalance2 = hre.ethers.utils.formatEther(
      await sbLending.ERC20DepositList(wethToken.address, metaMaskUserAddress)
    );
    const DAIdepositBalance2 = hre.ethers.utils.formatEther(
      await sbLending.ERC20DepositList(daiToken.address, metaMaskUserAddress)
    );
    const SBLdepositBalance2 = hre.ethers.utils.formatEther(
      await sbLending.ERC20DepositList(sblToken.address, metaMaskUserAddress)
    );
    ...
    assert(
      SBLdepositBalance2 > 50 &&
        DAIdepositBalance2 > 50 &&
        SBLdepositBalance2 > 50
    );
  });
  //Correct Math on Contract
  it("Test Function calculateMaxBorrow(address _user)", async function () {
    const totalDeposit = hre.ethers.utils.formatEther(
      await sbLending.calculateTotalDeposit(metaMaskUserAddress)
    );
  ...
  it("Test Borrow Function", async function () {
    ...
    const WETHborrowBalance = hre.ethers.utils.formatEther(
      await sbLending.ERC20BorrowList(wethToken.address, metaMaskUserAddress)
    );
    const DaiBorrowBalance = hre.ethers.utils.formatEther(
      await sbLending.ERC20BorrowList(daiToken.address, metaMaskUserAddress)
    );
    const SBLBorrowBalance = hre.ethers.utils.formatEther(
      await sbLending.ERC20BorrowList(sblToken.address, metaMaskUserAddress)
    );
    ...
    assert(
      WETHborrowBalance > 14 && DaiBorrowBalance > 14 && SBLBorrowBalance > 14
    );
  });
  it("Test PayBack Function", async function () {
    ...
    const WETHborrowBalance2 = hre.ethers.utils.formatEther(
      await sbLending.ERC20BorrowList(wethToken.address, metaMaskUserAddress)
    );
    const DaiBorrowBalance = hre.ethers.utils.formatEther(
      await sbLending.ERC20BorrowList(wethToken.address, metaMaskUserAddress)
    ...
    assert(
      WETHborrowBalance2 < 11 && DaiBorrowBalance < 11 && SBLBorrowBalance < 11
    );
  });

  it("Test LiquidateCall Function - Payback ETH get ETH back", async function () {
    const payBackAmount = hre.ethers.utils.parseEther("10");
    const WETHborrowBeforeLiquidation = hre.ethers.utils.formatEther(
      await sbLending.ERC20BorrowList(wethToken.address, metaMaskUserAddress)
    );
    const WETHdepositBeforeLiquidation = hre.ethers.utils.formatEther(
      await sbLending.ERC20DepositList(wethToken.address, metaMaskUserAddress)
    );
    await sbLending.liquidationCall(
      wethToken.address,
      wethToken.address,
      metaMaskUserAddress,
      payBackAmount
    );
    const WETHborrowAfterLiquidation = hre.ethers.utils.formatEther(
      await sbLending.ERC20BorrowList(wethToken.address, metaMaskUserAddress)
    );
    const WETHdepositAfterLiquidation = hre.ethers.utils.formatEther(
      await sbLending.ERC20DepositList(wethToken.address, metaMaskUserAddress)
    );
    ...
    assert(
      WETHdepositAfterLiquidation < WETHdepositBeforeLiquidation &&
        WETHborrowBeforeLiquidation < WETHborrowAfterLiquidation
    );
  });
});
